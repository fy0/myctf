...
class QuestionEdit(AdminView):
    ...
    def value_valid(self, qid):
        ...
        # id
        if id == '':
            self.messages.error('编号为必填')
        else:
            if not id.isdigit():
                self.messages.error('编号必须为数字')
            elif (qid is None or int(id) != int(qid)) and int(id) in Games.data:
                self.messages.error('该编号已经存在')
        ...
        # 分数
        if score == '':
            self.messages.error('分数为必填')
        else:
            if not id.isdigit():
                self.messages.error('分数必须为数字')
        ...
        def depend_check(prefix, txt):
            try:
                info = json.loads(txt)
                if type(info) != list:
                    self.messages.error('%s类型不为list' % prefix)
                else:
                    for i in info:
                        if type(i) != int:
                            self.messages.error('%s子项类型必须为整型' % prefix)
                            break
                        else:
                            if not int(i) in Games.data:
                                self.messages.error('%s中的编号%s不存在' % (prefix, i))
                                break
                if not self.messages.has_error():
                    return info
            except:
                self.messages.error('%s不是标准的json格式' % prefix)

        # 用户前置题
        _ = depend_check('用户前置题', depend)
        depend = _ if _ is not None else depend

        # 全局前置题
        _ = depend_check('全局前置题', depend_g)
    ...
    def get(self, qid):
        ...
        if not q:
            ...
            self.render('admin/question_edit.html', question=JsDict(q), title='编辑：%s' % q['title'])
    ...
    def post(self, qid):
        ...
        if self.messages.has_error():
            q = Games.data.get(int(qid))
            self.render('admin/question_edit.html', question=JsDict(ret), title='编辑：%s' % q['title'])
        else:
            ret['id'] = int(qid)
            Games.game_edit(ret)
...
class QuestionAdd(AdminView):
    def get(self):
        self.render('admin/question_edit.html', question=JsDict({}), title='添加题目')

    def post(self):
        ret = QuestionEdit.value_valid(self, None)

        if self.messages.has_error():
            self.render('admin/question_edit.html', question=JsDict(ret), title='添加题目', is_new_question=True)
        else:
            Games.game_add(ret)
            self.messages.success('添加题目成功')
            self.redirect(url_for('admin_questions'))
...
class QuestionRemove(AdminView):
    def get(self, qid):
        ...
        self.messages.success('题目%s已经删除！' % qid)
        self.redirect(url_for('admin'))
